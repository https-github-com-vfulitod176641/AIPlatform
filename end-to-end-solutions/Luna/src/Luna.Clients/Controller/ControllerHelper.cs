...
namespace Luna.Clients.Controller
{
    public static class ControllerHelper
    {
        ...
        public static async Task<List<Models.Controller.Backend.GetAllPublishedPipelinesResponse>> GetAllPublishedPipelines(AMLWorkspace workspace)
        {
            var region = await GetRegion(workspace);

            var requestUrl = $"https://{region}.api.azureml.ms/pipelines/v1.0" + workspace.ResourceId + $"/pipelines";
            var requestUri = new Uri(requestUrl);
            var request = new HttpRequestMessage { RequestUri = requestUri, Method = HttpMethod.Get };
            ...
            List<Models.Controller.Backend.GetAllPublishedPipelinesResponse> pipelines = (List<Models.Controller.Backend.GetAllPublishedPipelinesResponse>)System.Text.Json.JsonSerializer.Deserialize(responseContent, typeof(List<Models.Controller.Backend.GetAllPublishedPipelinesResponse>));
            if (pipelines == null)
            {
                throw new LunaServerException($"Query result in bad format. The response is {responseContent}.");
            }
        ...
        public static async Task<GetServiceKeysResponse> GetServiceKeys(Product product, Deployment deployment, APIVersion version, AMLWorkspace workspace, APISubscription apiSubscription, Guid endpointId)
        {
            var region = await GetRegion(workspace);
            ...
            GetServiceKeysResponse result = (GetServiceKeysResponse)System.Text.Json.JsonSerializer.Deserialize(responseContent, typeof(GetServiceKeysResponse));
            return result;
        }
        ...
        public static async Task<Models.Controller.GetAllRealTimeServiceEndpointsByUserProductAndDeploymentResponse> GetAllRealTimeServiceEndpointsByUserProductDeployment(Product product, Deployment deployment, APIVersion version, AMLWorkspace workspace, APISubscription apiSubscription)
        {
            ...
            foreach (var endpoint in endpoints)
            {
                var getServiceKeyResponse = await GetServiceKeys(product, deployment, version, workspace, apiSubscription, new Guid(endpoint.name));
                getAllRealTimeServiceEndpointsByUserProductAndDeploymentResponse.endpoints.Add(new Models.Controller.GetAllRealTimeServiceEndpointsByUserProductAndDeploymentResponse.Operation()
                {
                    ...
                    primaryKey = getServiceKeyResponse.primaryKey,
                    secondaryKey = getServiceKeyResponse.secondaryKey,
